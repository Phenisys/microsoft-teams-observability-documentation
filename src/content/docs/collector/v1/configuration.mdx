---
title: Configuration
description: Build a complete v1 config.yaml step by step, with guidance, tips, and a final full example.
sidebar:
  label: Configuration
  order: 2
---

import { Tabs, TabItem, Aside, Steps } from '@astrojs/starlight/components';

This page follows the same approach as the v2 guide: build your file by sections, understand what each part does, then use a complete example at the end.

## Required Sections

Your v1 `config.yaml` should contain these top-level sections:

- `microsoft_authentication`
- `output`
- `collection_config`

<Aside type="caution">
`CallRecords.Read.All`, `Reports.Read.All`, and `ServiceHealth.Read.All` application permissions are required before startup.
</Aside>

## 1. Build `microsoft_authentication`

Use one of these authentication approaches.

<Tabs>
  <TabItem label="Client Secret">
```yaml
microsoft_authentication:
    microsoft_tenant_id: 'your_tenant_id'
    microsoft_client_id: 'your_client_id'
    microsoft_client_secret: 'your_client_secret'
    microsoft_scope: 'https://graph.microsoft.com/.default'
    microsoft_grant_type: 'client_credentials'
```
  </TabItem>
  <TabItem label="Certificate">
```yaml
microsoft_authentication:
    microsoft_tenant_id: 'your_tenant_id'
    microsoft_client_id: 'your_client_id'
    microsoft_client_assertion_type: 'urn:ietf:params:oauth:client-assertion-type:jwt-bearer'
    microsoft_client_certificate_path: '/path/to/pem'
    microsoft_client_certificate_passphrase: 'my_passphrase'
    microsoft_scope: 'https://graph.microsoft.com/.default'
    microsoft_grant_type: 'client_credentials'
```
  </TabItem>
  <TabItem label="VAAC Add-on">
```yaml
microsoft_authentication:
    # add these only for Auto Attendant / Call Queue features
    microsoft_username: 'your_account'
    microsoft_password: 'your_password'
```
  </TabItem>
</Tabs>

<Aside type="tip">
VAAC credentials are needed only for:
`ms_teams_autoattendant_collection` and `ms_teams_callqueue_collection`.
</Aside>

## 2. Build `output`

Enable only the destinations you need.

<Tabs>
  <TabItem label="Dynatrace">
```yaml
output:
    dynatrace:
        enabled: true
        dynatrace_tenant_id: 'your_tenant_id'
        dynatrace_api_token: 'your_token'
```
  </TabItem>
  <TabItem label="Splunk">
```yaml
output:
    splunk:
        enabled: true
        splunk_hec_url: 'https://mysplunk:8088/services/collector/event'
        splunk_hec_token: 'hec_token'
        splunk_ssl_check: false
```
  </TabItem>
  <TabItem label="Console">
```yaml
output:
    console:
        enabled: true
```
  </TabItem>
</Tabs>

<Aside type="note">
At least one output must be enabled, otherwise the collector exits at startup.
</Aside>

## 3. Build `collection_config`

This section defines scheduling, logs, license path, and enabled features.

```yaml
collection_config:
    logging_services: ['dynatrace','splunk','console']
    microsoft_max_call_duration_hours: 5
    logfile_log_level: 'INFO'
    license_file: '/path/to/license.lic'
    interval_collection_minutes: 1
    features:
        ms_teams_calls_collection:
            enabled: true
        ms_teams_issues_collection:
            enabled: true
        ms_teams_pstn_calls_collection:
            enabled: true
        ms_teams_direct_routing_calls_collection:
            enabled: true
        ms_teams_site_definition:
            enabled: false
        ms_teams_autoattendant_collection:
            enabled: false
        ms_teams_callqueue_collection:
            enabled: false
```

<Aside type="tip">
`logging_services` should match enabled outputs. Example: if only Splunk is enabled, use `['splunk']`.
</Aside>

<Aside type="caution">
`DEBUG` can produce large logs and expose sensitive metadata. Use it only for short troubleshooting sessions.
</Aside>

## 4. Optional `sites.csv` Mapping

If `ms_teams_site_definition.enabled: true`, add:

```yaml
collection_config:
    features:
        ms_teams_site_definition:
            enabled: true
            site_definition_file: '/path/to/sites.csv'
```

The CSV must use this header order:

```text
subnet,site,country,lon,lat
```

If no valid CSV is loaded, calls appear as unmapped in site views.

## 5. Full Example (`config.yaml`)

Use this full template as a base and replace placeholders:

```yaml
microsoft_authentication:
    # Microsoft Graph API authentication configuration. Need Reports.Read.All, ServiceHealth.Read.All, CallRecords.Read.All permissions
    microsoft_tenant_id: 'your_tenant_id'
    microsoft_client_id: 'your_client_id'
    microsoft_client_secret: 'your_client_secret'
    # microsoft_client_assertion_type: 'urn:ietf:params:oauth:client-assertion-type:jwt-bearer'
    # microsoft_client_certificate_path: '/path/to/pem'
    # microsoft_client_certificate_passphrase: 'my_passphrase. Delete field if not used'
    microsoft_scope: 'https://graph.microsoft.com/.default'
    microsoft_grant_type: 'client_credentials'
    
    #VAAC Account. Documentation : https://learn.microsoft.com/en-us/microsoftteams/aa-cq-cqd-historical-reports#access-permissions
    microsoft_username: 'your_account'
    microsoft_password: 'your_password'

output:
    dynatrace:
        enabled: false # Set to true if you want to send data to Dynatrace
        dynatrace_tenant_id: 'your_tenant_id' # Enter your tenant ID only (not the full URL), e.g., https://<tenant_id>.apps.dynatrace.com
        dynatrace_api_token: 'your_token' # Need log.ingest permission
    splunk:
        enabled: false # Set to true if you want to send data to Splunk
        splunk_hec_url: 'hec_url' # Ex: 'https://mysplunk:8088/services/collector/event'
        splunk_hec_token: 'hec_token'
        splunk_ssl_check: false
    console:
        enabled: false # Debug option : Set to true if you want to only test this script without sending any data elsewhere

collection_config:
    logging_services: ['dynatrace','splunk','console'] # Available services: dynatrace, splunk, console
    microsoft_max_call_duration_hours: 5
    logfile_log_level: 'INFO' # Be careful with debug mode, there's a lot of written logs and sensitive data!
    # License path format example:
    # - Windows : 'C:\Program Files\Phenisys\license.lic' or '%ProgramFiles%\Phenisys\license.lic'
    # - Linux : '/opt/phenisys/license.lic'
    license_file: '/path/to/license.lic'
    interval_collection_minutes: 1
    features:
        ms_teams_calls_collection:
            enabled: true
        ms_teams_issues_collection:
            enabled: true
        ms_teams_pstn_calls_collection:
            enabled: true
        ms_teams_direct_routing_calls_collection:
            enabled: true
        ms_teams_site_definition:
            enabled: true
            # Site lookup definition from CSV file. Format must be comma separated value with these headers : subnet,site,country,lon,lat
            # Ex : 
            # subnet,site,country,lon,lat
            # 10.1.0.0,Paris_HQ,FR,43.1003,5.8384
            site_definition_file: '/path/to/sites.csv' 
        ms_teams_autoattendant_collection:
            enabled: true
        ms_teams_callqueue_collection:
            enabled: true
```

## 6. Validate Before Production Start

<Steps>
1. Check the binary and config path:

   ```bash
   ./ms-teams-agent.bin --version
   ```

2. Run one first cycle without historical state:

   ```bash
   ./ms-teams-agent.bin --config ./conf/config.yaml --ignore_state
   ```

3. Check logs and backend ingestion before enabling service mode.
</Steps>
