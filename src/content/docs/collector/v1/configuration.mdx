---
title: Configuration
description: Canonical v1 configuration for the legacy collector.
sidebar:
  label: Configuration
  order: 2
---
...
Use this page as the canonical `config.yaml` reference for **collector v1**.

## Reference Configuration (v1)
...
<Aside type="note">
Enable at least one output backend by setting `output.<backend>.enabled: true`, then keep `collection_config.logging_services` aligned with the enabled outputs.
</Aside>
