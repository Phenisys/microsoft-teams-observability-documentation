---
title: Installation
description: Install and start the v1 collector from prerequisites to first successful ingestion.
sidebar:
  label: Installation
  order: 1
---
...
2. Download the v1 binary (`ms-teams-agent.bin` or `ms-teams-agent.exe`) and template config (`conf/template_config.yaml`).
...
<Aside type="note">
v1 is a standalone binary workflow (no `run`, `validate`, or `test-connection` sub-commands).
</Aside>
...
flowchart TD
    ...
    E --> D
    D -->|OK| F["run --config (production loop)"]
```
