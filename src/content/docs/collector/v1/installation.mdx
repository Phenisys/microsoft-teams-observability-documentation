...
1. Configure Microsoft Graph permissions (see [Azure Permissions](/collector/azure-permissions/)).
2. Download the collector binary (`ms-teams-agent.bin` or `ms-teams-agent.exe`) and template config (`conf/template_config.yaml`) from the [latest GitHub release](https://github.com/Phenisys/microsoft-teams-observability/releases/latest).
3. Obtain your license file and note its absolute path for `collection_config.license_file`.

Collector source repository: [microsoft-teams-observability](https://github.com/Phenisys/microsoft-teams-observability)
...
Full settings reference: [Configuration](/collector/v1/configuration/)
...
See [Service Management](/collector/v1/service/) for service lifecycle operations.
...
<Aside type="tip">
If no data appears after one interval, use [Collector Troubleshooting](/collector/troubleshooting/).
</Aside>
