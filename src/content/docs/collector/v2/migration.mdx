...
v2 does not read the v1 JSON state (`state/state_<tenant>.json`). There is no automatic converter. On first v2 startup, starting with a clean state is the recommended path:
