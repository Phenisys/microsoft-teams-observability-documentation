...
flowchart TD
    ...
    B --> C["New App Registration → tenant_id + client_id"]
    C --> D{Auth method?}
    D -->|Client Secret| E[Create client secret]
    D -->|Certificate PEM| F["Upload public cert\nStore PEM on host"]
    E --> G[Add Graph API Permissions]
    F --> G
    G --> H[Grant admin consent]
    H --> I[Copy credentials to config.yaml]
```
...
For all available configuration fields, see [Configuration Reference](/collector/v2/configuration/).
