...
Dynatrace only installs custom extensions that are signed by a trusted root. The MS Teams Observability extension is signed with the Phenisys developer root, so the matching certificate must be present on every ActiveGate that will run the extension. Without it, the upload in Step 2 will be rejected.
...
<Aside type="note">
Repeat this step on every ActiveGate in the group selected in Step 3. For background on extension signing, see the [Dynatrace documentation on signing extensions](https://docs.dynatrace.com/docs/ingest-from/extensions/develop-your-extensions/sign-extensions).
</Aside>
