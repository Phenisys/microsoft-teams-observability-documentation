...
import dynatraceExtensionConfigurationScreen from '../../../../assets/images/dynatrace-extension-configuration_screen.png';
...
- The **Phenisys root certificate** (`ca.pem`) — <a href={`${import.meta.env.BASE_URL}files/ca.pem`} download>Download ca.pem</a>
...
<Steps>
1. Download the Phenisys root certificate: <a href={`${import.meta.env.BASE_URL}files/ca.pem`} download>Download ca.pem</a>

2. Copy the certificate to each ActiveGate host in the target group.

   <Aside type="caution">
   There may already be a `ca.pem` file on the ActiveGate. **Rename the downloaded file** (e.g. `phenisys-ca.pem`) before copying to avoid overwriting it.
   </Aside>
...
<figure>
  <img src={dynatraceExtensionConfigurationScreen.src} alt="Dynatrace extension configuration screen showing Microsoft Cloud Deployment and Custom CA Bundle Path fields." />
  <figcaption>Dynatrace extension configuration screen.</figcaption>
</figure>
...
| `microsoft_authentication.cloud_deployment` | **Microsoft Cloud Deployment** | Default `global`. Options: `global`, `us_gov_gcc_high`, `us_gov_dod`, `china` |
...
| YAML field | Extension UI field | Notes |
|---|---|---|
| `advanced.ca_bundle_path` | **Custom CA Bundle Path** | Optional. Absolute path to a PEM CA bundle on the ActiveGate host (for TLS-inspecting proxies or enterprise custom CA chains). |
