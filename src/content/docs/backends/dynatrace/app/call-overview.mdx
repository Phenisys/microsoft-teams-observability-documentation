...
Reach this page by clicking a **call ID** in the [Calls](../calls/) page. Only calls with a duration > 0 have a clickable link.
...
<Aside type="note">
The download button is **disabled** if Microsoft credentials are not configured in [Configuration](../configuration/).
</Aside>
...
<Aside type="note">
Both **Microsoft configuration** and **AI Analysis configuration** must be set up in [Configuration](../configuration/) for these buttons to be enabled.
</Aside>
...
<Aside type="note">
Requires Microsoft credentials configured in [Configuration](../configuration/). The button is disabled otherwise.
</Aside>
