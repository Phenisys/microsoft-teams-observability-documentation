...
You access this page by clicking a call ID from <a href={`${import.meta.env.BASE_URL}backends/dynatrace/app/calls/`}>Calls</a>. Only calls with recording data (duration > 0) are clickable.
...
<Aside type="note">
Download is disabled if Microsoft credentials are not configured in <a href={`${import.meta.env.BASE_URL}backends/dynatrace/app/configuration/`}>Configuration</a>.
</Aside>
...
<Aside type="note">
Both Microsoft and AI panels must be configured in <a href={`${import.meta.env.BASE_URL}backends/dynatrace/app/configuration/`}>Configuration</a>.
</Aside>
