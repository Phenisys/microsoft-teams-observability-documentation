...
| **ID** | No | Unique call identifier. Link to [Call Overview](../call-overview/) only when details are available |
...
- **Details available + duration > 0**: ID is a clickable link to [Call Overview](../call-overview/). The link includes a 10-minute padding before the start and after the end.
...
| Action | Destination |
|--------|-------------|
| Click a call ID (with recording) | [Call Overview](../call-overview/) for that call |
