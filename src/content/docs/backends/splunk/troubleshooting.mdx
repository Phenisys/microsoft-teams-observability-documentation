...
**Checks:**
1. Run `ms-teams-agent run --config ./config.yaml --ignore-state` and confirm export succeeds.
...
2. Verify `sites.csv` is configured in the <a href={`${import.meta.env.BASE_URL}collector/v2/configuration/#optional-sitescsv-mapping`}>Configuration</a>.

<Aside type="tip">
For collector-side issues (Microsoft Graph authentication, collection errors), see <a href={`${import.meta.env.BASE_URL}collector/troubleshooting/`}>Collector Troubleshooting</a>.
</Aside>
